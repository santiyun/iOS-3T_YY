...
- (instancetype)initWithFrame:(CGRect)frame andContext:(OFHandle)context {
    ...
    _selectBorder = nil;
    _context = context;
    _beautyUtil = new BeautyUtil();
    
    std::string dataPath = [[[NSBundle mainBundle] bundlePath] UTF8String];
    std::string effectPath = dataPath + "/effects/beauty.zip";
    _beautyUtil->LoadEffect(_context, effectPath);
...
- (void)initScroll {
    ...
    for (int i = 0; i < titles.count; ++i) {
        ...
        //[NSBundle.mainBundle pathForResource:icons[i] ofType:nil];
...
- (void)onSliderValueChange:(UISlider*)slider {
    ...
    _beautyUtil->SetBeautyOptionValue(_select - 1, value);
}
...
- (void)onEnableBeautyEffect:(bool)enable {
    _beautyUtil->EnableEffect(enable);
}
