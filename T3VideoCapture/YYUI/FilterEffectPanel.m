...
- (instancetype)initWithFrame:(CGRect)frame andContext:(OFHandle)context {
    ...
    _selectBorder = nil;
    _context = context;
    _filterUtil = [[FilterTool alloc] init];
...
- (void)loadFilter:(NSString*)path {
    ...
    NSString *effectPath = [NSBundle.mainBundle pathForResource:path ofType:nil];
    [_filterUtil loadEffect:_context path:effectPath];
}
