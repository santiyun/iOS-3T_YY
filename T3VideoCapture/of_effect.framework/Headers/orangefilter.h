 ...
 * of failure
 * @param[in] modelPath, path of human action model files, e,g "d:/venus_models"
 * if you're not going to use human action, set to OF_NULL.
 * @return, OF_Result_Success in case of success, others in case of failure.
 */
OF_API OF_Result OF_CreateContext(
    OFHandle* contextID,                        /* [out] */
    const char* modelPath);                     /* [in] */
